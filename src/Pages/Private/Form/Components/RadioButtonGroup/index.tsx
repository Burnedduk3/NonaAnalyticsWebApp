        ...
        <div className="header-container">
          ...
          {stackPhrase && (
            <div className="radio-buttons-header">
              {items.map((header) => {
                return (
                  <div key={`${header.name}-div`} className="text-header-container">
                    <h4>{header.name}</h4>
                  </div>
                );
              })}
          ...
          <div className="option-container">
            {items.map((option) => (
              <label
                ...
                key={`${questionId}-${option.name}-label`}
              ...
              >
                <input
                  ...
                  value={option.name}
                  defaultChecked={
                    questionAnswer ? questionAnswer.answer === option.name : false
                  }
                  onClick={() =>
                    setResponse(option.name, questionId, order, inputConfirmation)
                  }
                />
